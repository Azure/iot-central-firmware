                    ...
                    const char *authHeader, char *operationId, char *hostName)
{
    ...
    if (hostName == NULL)
    {
        ...
        size = snprintf(NULL, 0, "{\"registrationId\":\"%s\"}", deviceId);
...
\r\n"),
    ...
    while (size > total)
    {
        int sent = client->write((unsigned char *)*tmpBuffer + total, strlen(*tmpBuffer + total), 100);
        if (sent < 0)
        {
    ...
    if (socketReadThemAll(client, tmpBuffer) != 0)
        goto error_exit;
