...
static EVENT_INSTANCE *createEventInstance(IOTContextInternal *internal,
  ...
  *errorCode = 0;
  ...
  EVENT_INSTANCE *currentMessage =
      (EVENT_INSTANCE *)malloc(sizeof(EVENT_INSTANCE));
  ...
  if (currentMessage->messageHandle == NULL) {
    IOTC_LOG(
        F("ERROR: (iotc_send_telemetry) IoTHubMessage_CreateFromByteArray has "
          "failed."));
    free(currentMessage);
    *errorCode = 9;
    return NULL;
...
static void sendConfirmationCallback(IOTHUB_CLIENT_CONFIRMATION_RESULT result,
  ...
  if (internal->callbacks[/*IOTCallbacks::*/ MessageSent].callback) {
    if (IOTHUB_CLIENT_RESULT::IOTHUB_CLIENT_OK !=
        IoTHubMessage_GetByteArray(eventInstance->messageHandle, &buffer,
                                   &size)) {
      IOTC_LOG(
          F("ERROR: (sendConfirmationCallback) IoTHubMessage_GetByteArray has "
            "failed."));
    }
...
#define CONVERT_TO_IOTHUB_MESSAGE(x)                          \
  (x == IOTC_MESSAGE_ACCEPTED                                 \
       ? IOTHUBMESSAGE_ACCEPTED                               \
       : (x == IOTC_MESSAGE_REJECTED ? IOTHUBMESSAGE_REJECTED \
...
static int onCommand(const char *method_name, const unsigned char *payload,
                     size_t size, unsigned char **response, size_t *resp_size,
                     void *userContextCallback) {
...
#define CONVERT_TO_IOTC_CONNECT_ENUM(x)                                                 \
  (x == IOTHUB_CLIENT_CONNECTION_EXPIRED_SAS_TOKEN                                      \
       ? IOTC_CONNECTION_EXPIRED_SAS_TOKEN                                              \
       : (x == IOTHUB_CLIENT_CONNECTION_RETRY_EXPIRED                                   \
              ? IOTC_CONNECTION_RETRY_EXPIRED                                           \
              : (x == IOTHUB_CLIENT_CONNECTION_DEVICE_DISABLED                          \
                     ? IOTC_CONNECTION_DEVICE_DISABLED                                  \
                     : (x == IOTHUB_CLIENT_CONNECTION_BAD_CREDENTIAL                    \
                            ? IOTC_CONNECTION_BAD_CREDENTIAL                            \
                            : (x == IOTHUB_CLIENT_CONNECTION_NO_NETWORK                 \
                                   ? IOTC_CONNECTION_NO_NETWORK                         \
                                   : (x == IOTHUB_CLIENT_CONNECTION_COMMUNICATION_ERROR \
                                          ? IOTC_CONNECTION_COMMUNICATION_ERROR         \
                                          : IOTC_CONNECTION_OK))))))
...
void echoDesired(IOTContextInternal *internal, const char *propertyName,
  ...
  AzureIOT::JSObject propertyNameObject, desiredObject,
      desiredObjectPropertyName;
...
static void deviceTwinGetStateCallback(DEVICE_TWIN_UPDATE_STATE update_state,
  ...
  if (update_state == DEVICE_TWIN_UPDATE_PARTIAL &&
      payloadObject.getNameAt(0) != NULL) {
    callDesiredCallback(internal, payloadObject.getNameAt(0),
    ...
    // if they don't match then call the associated callback for the desired
    // property
    ...
    for (unsigned i = 0, count = desired.getCount(); i < count; i++) {
      ...
      if (itemName != NULL && itemName[0] != '$') {
        ...
        const char *version = NULL, *desiredVersion = NULL, *value = NULL,
                   *desiredValue = NULL;
        ...
        if (containsKey && strcmp(desiredVersion, version) == 0) {
          ...
          IOTC_LOG(F("key: %s either not found in reported or versions do not "
                     "match\r\n"),
                   itemName);
          AzureIOT::JSObject itemValue;
          if (desired.getObjectAt(i, &itemValue) &&
          ...
          echoDesired(internal, itemName, (const char *)payLoad, "completed",
                      200);
        }
      }
    }
...
static void registation_status_callback(PROV_DEVICE_REG_STATUS reg_status,
                                        void *user_context) {
...
static void register_device_callback(PROV_DEVICE_RESULT register_result,
                                     ...
                                     const char *deviceId, void *user_context) {
  if (user_context == NULL) {
    ...
    if (register_result == PROV_DEVICE_RESULT_OK) {
      ...
      IOTC_LOG(
          F("ERROR: (register_device_callback) Failure encountered on "
            "registration!"));
...
int iotc_connect(IOTContext ctx, const char *scope, const char *keyORcert,
  ...
  if (type == IOTC_CONNECT_CONNECTION_STRING) {
    ...
    if ((handle = Prov_Device_LL_Create(
             internal->endpoint == NULL ? DEFAULT_ENDPOINT : internal->endpoint,
             scope, Prov_Device_MQTT_Protocol)) == NULL) {
    ...
    if (Prov_Device_LL_SetOption(handle, "TrustedCerts", certificates) !=
        PROV_DEVICE_RESULT_OK) {
      IOTC_LOG(
          F("ERROR: (iotc_connect) Failed to set option \"TrustedCerts\"."));
      errorCode = 1;
      goto fnc_exit;
    } else
#endif  // defined(MBED_BUILD_TIMESTAMP) || defined(TARGET_MXCHIP_AZ3166)
        if (Prov_Device_LL_Register_Device(
                handle, register_device_callback, &user_ctx,
                registation_status_callback,
                &user_ctx) != PROV_DEVICE_RESULT_OK) {
      IOTC_LOG(
          F("ERROR: (iotc_connect) Failed calling "
  ...
  if (pos == 0 || pos >= AZ_IOT_HUB_MAX_LEN) {
    IOTC_LOG(
        F("ERROR: (iotc_connect) connection information is out of buffer."));
    errorCode = 15;
    goto fnc_exit;
  }
  ...
  if ((internal->clientHandle = IoTHubClient_LL_CreateFromConnectionString(
    ...
    IOTC_LOG(
        F("ERROR: (iotc_connect) Couldn't create hub from connection string."));
    errorCode = 4;
    goto fnc_exit;
  }

  IoTHubClient_LL_SetRetryPolicy(internal->clientHandle,
                                 IOTHUB_CLIENT_RETRY_EXPONENTIAL_BACKOFF, 1200);

#if defined(MBED_BUILD_TIMESTAMP) || defined(TARGET_MXCHIP_AZ3166)
  if (IoTHubClient_LL_SetOption(
          internal->clientHandle, "TrustedCerts",
          certificates /* src/cores/arduino/az_iot/azureiotcerts.h */) !=
      IOTHUB_CLIENT_OK) {
    IOTC_LOG(
        F("ERROR: Failed to set option \"TrustedCerts\" "
          "IoTHubClient_LL_SetOption failed."));
    return 5;
  }
#endif  // defined(MBED_BUILD_TIMESTAMP) || defined(TARGET_MXCHIP_AZ3166)
  ...
  if (IoTHubClient_LL_SetDeviceTwinCallback(internal->clientHandle,
                                            deviceTwinGetStateCallback,
                                            internal) != IOTHUB_CLIENT_OK) {
    IOTC_LOG(
        F("ERROR: (iotc_connect) IoTHubClient_LL_SetXXXXCallback failed."));
    errorCode = 5;
    goto fnc_exit;
  }
  ...
  if (IoTHubClient_LL_SetConnectionStatusCallback(
          internal->clientHandle, connectionStatusCallback, internal) !=
      IOTHUB_CLIENT_OK) {
    IOTC_LOG(
        F("ERROR: (iotc_connect) IoTHubClient_LL_SetXXXXCallback failed. "
          "ERR:0x0005"));
...
int iotc_send_telemetry(IOTContext ctx, const char *payload, unsigned length) {
  ...
  EVENT_INSTANCE *currentMessage =
      createEventInstance(internal, payload, length, NULL, &errorCode);
  ...
  assert(outputLength && outputLength < 128 &&
         timeBuffer[outputLength - 1] == '\n');
  ...
  hubResult = IoTHubClient_LL_SendEventAsync(
      internal->clientHandle, currentMessage->messageHandle,
      sendConfirmationCallback, currentMessage);

  if (hubResult != IOTHUB_CLIENT_OK) {
    IOTC_LOG(
        F("ERROR: (iotc_send_telemetry) IoTHubClient_LL_SendEventAsync has "
          "failed => hubResult is (%d)."),
        hubResult);
    freeEventInstance(currentMessage);
    return 1;
  }
...
int iotc_send_property(IOTContext ctx, const char *payload, unsigned length) {
  ...
  IOTHUB_CLIENT_RESULT hubResult = IoTHubClient_LL_SendReportedState(
      internal->clientHandle, (const unsigned char *)payload, length,
      deviceTwinConfirmationCallback, currentMessage);

  if (hubResult != IOTHUB_CLIENT_OK) {
    IOTC_LOG(
        F("ERROR: (iotc_send_telemetry) IoTHubClient_LL_SendReportedState has "
          "failed => hubResult is (%d). ERROR:0x000B"),
        hubResult);
    freeEventInstance(currentMessage);
    return 11;
  }
...
int iotc_set_network_interface(void *networkInterface) {
  IOTC_LOG(F(
      "ERROR: (iotc_set_network_interface) is not needed for this platform."));
  return 1;
}
