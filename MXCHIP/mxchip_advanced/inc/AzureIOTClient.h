...
struct DirectMethodNode {
  ...
  DirectMethodNode() : methodName(NULL), payload(NULL), length(0), next(NULL) {}
...
class AzureIOTClient {
 ...
 public:
  ...
  bool wasInitialized() { return !hasError; }
  ...
  // no reason for ordered map here yet toolchain has a messed up references to
  // `ceil`. which is required by unordered_map
