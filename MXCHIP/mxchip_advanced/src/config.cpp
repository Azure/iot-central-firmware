...
void ConfigController::storeConnectionString(StringBuffer &connectionString) {
  EEPROMInterface eeprom;
  eeprom.write((uint8_t *)*connectionString, connectionString.getLength(),
               AZ_IOT_HUB_ZONE_IDX);
}

void ConfigController::storeIotCentralConfig(StringBuffer &iotCentralConfig) {
  EEPROMInterface eeprom;
  eeprom.write((uint8_t *)*iotCentralConfig, iotCentralConfig.getLength(),
               IOT_CENTRAL_ZONE_IDX);
}

// Unused. Left here for sample
void ConfigController::readWiFi(char *ssid, int ssidLen, char *password,
                                int passwordLen) {
  EEPROMInterface eeprom;
  ...
  if (strcmp(ssid, WIFI_NAME) != 0 || strcmp(password, WIFI_PASSWORD) != 0) {
    ...
    eeprom.write((uint8_t *)pass_buffer, sizeof(WIFI_PASSWORD),
                 WIFI_PWD_ZONE_IDX);
  }
#endif
}

void ConfigController::readConnectionString(char *connectionString,
  ...
  eeprom.read((uint8_t *)connectionString, AZ_IOT_HUB_MAX_LEN, 0,
              AZ_IOT_HUB_ZONE_IDX);
...
void ConfigController::storeKey(StringBuffer &auth, StringBuffer &scopeId,
  ...
  LOG_VERBOSE("STORE scope:%s regId:%s key:%s",
              scopeId.getLength() > 0 ? *scopeId : "", *regId, *key);
  if ((*auth)[0] == 'S') {
...
void ConfigController::readGroupSXKeyAndDeviceId(char *scopeId,
                                                 char *registrationId,
                                                 char *sas, char &atype) {
  ...
  atype =
      strncmp(buffer + (AZ_IOT_HUB_MAX_LEN - 4), "SSYM", 4) == 0 ? 'S' : ' ';

  if (atype == ' ' &&
      strncmp(buffer + (AZ_IOT_HUB_MAX_LEN - 4), "X509", 4) == 0)
    atype = 'X';
  else if (atype == ' ')
    atype = 'C';
  ...
  if (atype == 'C') {
    ...
    *registrationId = '\0';
    ...
    strcpy(sas, buffer + SAS_REG_ID_ENDS);
       ...
       // !defined(IOT_CENTRAL_USE_X509_SAMPLE_CERT)
...
void ConfigController::readIotCentralConfig(char *iotCentralConfig,
                                            uint32_t buffer_size) {
  ...
  readWiFi(ssid, 1024, pass,
           1024);  // to force resetting wifi creds into eeprom
#else              // COMPILE_TIME_DEFINITIONS_SET
  EEPROMInterface eeprom;
  eeprom.read((uint8_t *)iotCentralConfig, IOT_CENTRAL_MAX_LEN, 0,
              IOT_CENTRAL_ZONE_IDX);
...
void ConfigController::clearIotCentralEEPROM() {
  ...
  char cleanBuffer[AZ_IOT_HUB_MAX_LEN] = {0};
  eeprom.write((uint8_t *)cleanBuffer, IOT_CENTRAL_MAX_LEN,
               IOT_CENTRAL_ZONE_IDX);
}