...
bool WiFiController::initApWiFi() {
  ...
  WiFi.macAddress(mac);
  unsigned length =
      snprintf(apName, STRING_BUFFER_32 - 1, "AZ3166_%c%c%c%c%c%c",
               mac[0] % 26 + 65, mac[1] % 26 + 65, mac[2] % 26 + 65,
               mac[3] % 26 + 65, mac[4] % 26 + 65, mac[5] % 26 + 65);
  ...
  // passcode below is not widely compatible. See the password is used as
  // pincode for onboarding
...
bool WiFiController::getIsConnected() { return WiFi.status() == WL_CONNECTED; }

String *WiFiController::getWifiNetworks(int &count) {
  LOG_VERBOSE("WiFiController::getWifiNetworks");
  String foundNetworks = "";  // keep track of network SSID so as to remove
                              // duplicates from mesh and repeater networks
...
void WiFiController::displayNetworkInfo() {
  ...
  WiFi.macAddress(mac);
  unsigned length = snprintf(
      buff, STRING_BUFFER_128,
      "WiFi:\r\n%s\r\n%s\r\nmac:%02X%02X%02X%02X%02X%02X", WiFi.SSID(),
      ip.get_address(), mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
