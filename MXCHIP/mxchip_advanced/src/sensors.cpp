...
float SensorController::readTemperature() {
  ...
  if (tempHumidity == NULL) {
    LOG_ERROR(
        "Trying to do readTemperature while the sensor wasn't initialized.");
    return 0xFFFF;
  }
...
void SensorController::readMagnetometer(int *axes) {
  ...
  if (magnetometer == NULL) {
    LOG_ERROR(
        "Trying to do readMagnetometer while the sensor wasn't initialized.");
    hasFailed = true;
...
void SensorController::readAccelerometer(int *axes) {
  ...
  if (accelGyro == NULL) {
    LOG_ERROR(
        "Trying to do readAccelerometer while the sensor wasn't initialized.");
    hasFailed = true;
  }
...
void SensorController::readGyroscope(int *axes) {
  ...
  if (accelGyro == NULL) {
    LOG_ERROR(
        "Trying to do readGyroscope while the sensor wasn't initialized.");
    hasFailed = true;
  }
...
bool SensorController::checkForShake() {
  ...
  if (accelGyro == NULL) {
    LOG_ERROR(
        "Trying to do checkForShake while the sensor wasn't initialized.");
    return false;
  }
