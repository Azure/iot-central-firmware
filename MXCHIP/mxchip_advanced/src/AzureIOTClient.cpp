...
void onCommand(IOTContext ctx, IOTCallbackInfo *callbackInfo) {
  LOG_VERBOSE("AzureIOTClient::onCommand (methodName:%s)",
              callbackInfo->tag != NULL ? callbackInfo->tag : "None");
  ...
  if (client != NULL) {
    ...
    if (it != client->methodCallbacks.end()) {
      ...
      assert(pcopy);
      memcpy(pcopy, (const char *)callbackInfo->payload,
...
void onConnectionStatus(IOTContext ctx, IOTCallbackInfo *callbackInfo) {
  LOG_VERBOSE("AzureIOTClient::connectionStatusCallback result:%d",
...
void onSettingsUpdated(IOTContext ctx, IOTCallbackInfo *callbackInfo) {
  ...
  if (it != client->methodCallbacks.end()) {
    ...
    callbackInfo->callbackResponse =
        strdup("TargetNotFound");  // iotc will free the memory
    LOG_ERROR("Property Name '%s' is not found @callDesiredCallback",
              *propName);
  }
}
...
void AzureIOTClient::init() {
  ...
  iotc_set_logging(IOTHUB_TRACE_LOG_ENABLED ? IOTC_LOGGING_ALL
                                            : (SERIAL_VERBOSE_LOGGING_ENABLED
                                                   ? IOTC_LOGGING_API_ONLY
                                                   : IOTC_LOGGING_DISABLED));

  ConfigController::readGroupSXKeyAndDeviceId(scopeId, registrationId,
                                              stringBuffer, atype);
  LOG_VERBOSE("SCOPEID: %s REGID: %s KEY: %s AuthType: %c", scopeId,
              registrationId, stringBuffer, atype);
  ...
  memcpy(stringBuffer, IOT_CENTRAL_CONNECTION_STRING,
         strlen(IOT_CENTRAL_CONNECTION_STRING));
#else
  switch (atype) {
    case 'C':
      connectType = IOTC_CONNECT_CONNECTION_STRING;
...
#endif
  errorCode =
      iotc_connect(context, scopeId, stringBuffer, registrationId, connectType);
  assert(errorCode == 0);
...
bool AzureIOTClient::registerCallback(const char *methodName,
