...
void TelemetryController::initializeTelemetryController(
    const char *iotCentralConfig) {
...
void TelemetryController::loop() {
  ...
  const uint32_t currentMillis =
      millis();  // reduce number of times we call this
  ...
  // when the A button is pressed the device state rotates to the next value and
  // a state telemetry message is sent
  ...
  if (canSend() &&
      currentMillis - lastTelemetrySend >= TELEMETRY_SEND_INTERVAL) {
    ...
    if (++sendCount % 6 == 0) {
      ...
      if (iotClient->sendReportedProperty(locationString)) {
        LOG_VERBOSE("Reported property location successfully sent %s",
                    locationString);
        StatsController::incrementReportedCount();
      } else {
  ...
  // example of sending a device twin reported property when the accelerometer
