...
void OnboardingController::loop() {
  if (!initializeCompleted) {
    ...
    snprintf(buffer, STRING_BUFFER_32, "PIN CODE %s",
             Globals::wiFiController.getPassword());
      ...
      if (client.available()) {
        ...
        if (c == '\n' && currentLineIsBlank) {
          ...
          if (requestMethod.startsWith("GET / ")) {
            ...
            client.write((uint8_t *)HTTP_MAIN_PAGE_RESPONSE,
                         sizeof(HTTP_MAIN_PAGE_RESPONSE) - 1);
            ...
            if (setupCompleted) {
              ...
              Screen.print(3, "         now :)");
              client.write((uint8_t *)HTTP_COMPLETE_RESPONSE,
                           sizeof(HTTP_COMPLETE_RESPONSE) - 1);
              resetController.initialize(3000);
              resetController.reset();
            } else {
              LOG_ERROR(
                  "User has landed on COMPLETE page without actually "
                  "completing the setup. Writing the START page to client.");
              processStartRequest(client);
            }
          } else {
            // 404
            LOG_VERBOSE("Request to %s -> 404!", request.c_str());
            client.write((uint8_t *)HTTP_404_RESPONSE,
...
void OnboardingController::processStartRequest(WiFiClient &client) {
  ...
  if (networks == NULL) {
    ...
    client.write((uint8_t *)HTTP_ERROR_PAGE_RESPONSE,
                 sizeof(HTTP_ERROR_PAGE_RESPONSE) - 1);
    return;
  }
...
void OnboardingController::processResultRequest(WiFiClient &client,
                                                String &request) {
  String data =
      request.substring(request.indexOf('?') + 1, request.indexOf(" HTTP/"));
    ...
    switch (idx) {
      ...
      case 5: {
        if (strncmp(key, "ACCEL", 5) == 0) {
          ...
          if (valueLength > 4 ||
              strncmp(value, Globals::wiFiController.getPassword(), 4) != 0) {
            LOG_ERROR("WRONG PIN CODE %s %s", value,
                      Globals::wiFiController.getPassword());
          } else {
            pincodePasses = true;
          }
  ...
  if (ssid.getLength() == 0 || (*(*auth) == 'S' && sasKey.getLength() == 0)) {
    ...
    client.write((uint8_t *)HTTP_REDIRECT_WRONG_PINCODE,
                 sizeof(HTTP_REDIRECT_WRONG_PINCODE) - 1);
    return;
  }
  ...
  client.write((uint8_t *)HTTP_REDIRECT_RESPONSE,
               sizeof(HTTP_REDIRECT_RESPONSE) - 1);
}